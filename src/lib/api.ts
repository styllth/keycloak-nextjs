...
async function refreshAccessToken(ctx: any = undefined): Promise<string> {
  const { "@myapp:refresh_token": refreshToken } = parseCookies(ctx);
  ...
  setCookie(ctx, "@myapp:access_token", data.access_token, {
    path: "/",
  });

  setCookie(ctx, "@myapp:refresh_token", data.refresh_token, {
    path: "/",
...
export function setupAPIClient(ctx: any = undefined) {
  const { "@myapp:access_token": token } = parseCookies(ctx);
